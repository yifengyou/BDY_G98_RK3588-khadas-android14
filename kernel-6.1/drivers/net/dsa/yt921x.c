...
/*
 * BSP 6.1: <net/dscp.h> and <net/ieee8021q.h> don't exist yet (the IETF
 * DSCP mapping helpers landed in 6.9). Provide the subset this driver
 * uses; the mapping is identical to the upstream RFC 8325 one.
 */
#define DSCP_MAX	64
...
#define IEEE8021Q_TT_BK	YT921X_TT_BK

static inline int ietf_dscp_to_ieee8021q_tt(u8 dscp)
{
	switch (dscp) {
	case 0:					/* CS0 */
	case 10: case 12: case 14:		/* AF11-AF13 */
		return YT921X_TT_BE;
	case 8:					/* CS1 */
		return YT921X_TT_BK;
	case 16:				/* CS2 */
	case 18: case 20: case 22:		/* AF21-AF23 */
		return YT921X_TT_EE;
	case 24:				/* CS3 */
	case 26: case 28: case 30:		/* AF31-AF33 */
		return YT921X_TT_CA;
	case 32:				/* CS4 */
	case 34: case 36: case 38:		/* AF41-AF43 */
		return YT921X_TT_VI;
	case 40:				/* CS5 */
	case 44:				/* VOICE-ADMIT */
	case 46:				/* EF */
...
/* BSP 6.1: container_of_const() was added in 6.4 */
...
static void yt921x_poll_mib(struct work_struct *work)
{
	...
	/* BSP 6.1: re-arm only if remove() hasn't run (see mib_stop) */
	if (!READ_ONCE(pp->mib_stop))
...
static int yt921x_mtu_fetch(struct yt921x_priv *priv, int port)
{
	...
	/* BSP 6.1: dsa_port has .slave, not .user */
		  ...
		  struct netlink_ext_ack *extack)
{
	...
	if (flags & YT921X_MARKER_PKT_MODE)
		...
		burst_sug += ETH_HLEN + yt921x_mtu_fetch(priv, port) +
			     ETH_FCS_LEN;
	if (burst_sug > burst)
		/* BSP 6.1: no NL_SET_ERR_MSG_FMT_MOD() (added in 6.4) */
		NL_SET_ERR_MSG_MOD(extack,
				   "Consider a burst at least frame-size plus headers");
				       ...
				       struct flow_cls_offload *cls)
{
	...
	if (flow_rule_match_key(rule, FLOW_DISSECTOR_KEY_CONTROL)) {
		...
		/* BSP 6.1: flow_rule_is_supp_control_flags() doesn't exist yet */
		if ((supp_flags & match.mask->flags) != match.mask->flags) {
			NL_SET_ERR_MSG_MOD(extack,
					   "Unsupported flags in control key");
			return -EOPNOTSUPP;
		}
				      ...
				      struct yt921x_priv *priv, int port)
{
	...
	flow_action_for_each(i, act, flow_action)
		...
		default:
fallback:
			/* BSP 6.1: flow_cls_common_offload has no skip_sw;
			 * every rule reaching the driver is hardware-offloaded.
			 * (the ';' keeps clang happy: fallthrough may not
			 * directly follow a label)
			 */
			;
			fallthrough;
	...
	/* BSP 6.1: no skip_sw; rules here are always hardware-offloaded */
	ruleext->r.sw_assisted = true;
	return 0;
}
	       ...
	       struct netlink_ext_ack *extack)
{
	...
	aclblk = priv->acl_blks[blkid];
	if (!aclblk) {
		/* BSP 6.1: kvzalloc_obj() doesn't exist yet (added in 6.9) */
...
yt921x_dsa_set_ageing_time(struct dsa_switch *ds, unsigned int msecs)
{
	...
	ctrl = clamp_t(u32, msecs / 5000, 1, U16_MAX);
...
static int yt921x_chip_setup_tc(struct yt921x_priv *priv)
{
	...
	ctrl = max_t(u32, priv->meter_slot_ns / op_ns, YT921X_METER_SLOT_MIN);
	res = yt921x_reg_write(priv, YT921X_METER_SLOT, ctrl);
	if (res)
		return res;
	priv->meter_slot_ns = ctrl * op_ns;

	ctrl = max_t(u32, priv->port_shape_slot_ns / op_ns,
...
static int yt921x_led_profile_setup(struct yt921x_priv *priv)
{
	...
	dsa_switch_for_each_user_port(dp, ds) {
		...
		dev_info(dev,
			 ...
			 dp->index, old0, (u32)YT921X_LED_PROFILE_LINK, old1,
			 (u32)YT921X_LED_PROFILE_ACT);
	}
...
static const struct dsa_switch_ops yt921x_dsa_switch_ops = {
	...
	/* eee */
	/* BSP 6.1: dsa_switch_ops has no .support_eee / dsa_supports_eee */
	...
	/* hsr */
	/* BSP 6.1: dsa_port_simple_hsr_join/leave helpers don't exist */
	/* mirror */
...
#if IS_ENABLED(CONFIG_DCB)
	/* dcb */
	/* BSP 6.1: dsa_switch_ops has no .port_get/set_apptrust */
...
static void yt921x_mdio_remove(struct mdio_device *mdiodev)
{
	...
	for (size_t i = ARRAY_SIZE(priv->ports); i-- > 0; ) {
		...
		/* BSP 6.1: no disable_delayed_work_sync(); stop re-arming,
		 * then wait for a possibly running instance.
		 */
		WRITE_ONCE(pp->mib_stop, true);
...
static int yt921x_mdio_probe(struct mdio_device *mdiodev)
{
	...
	/* BSP 6.1: dsa_switch has no dscp_prio_mapping_is_global */
	ds->priv = priv;
