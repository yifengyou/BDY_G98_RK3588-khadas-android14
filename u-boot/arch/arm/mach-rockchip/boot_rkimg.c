...
void setup_download_mode(void)
{
	...
	printf("DEBUG: setup_download_mode: dnl_key=%d, hotkey_dnl=%d\n",
	       rockchip_dnl_key_pressed(), is_hotkey(HK_ROCKUSB_DNL));
	...
	if (rockchip_dnl_key_pressed() || is_hotkey(HK_ROCKUSB_DNL)) {
		...
		printf("DEBUG: setup_download_mode: fastboot hotkey, setting reboot_mode=fastboot\n");
